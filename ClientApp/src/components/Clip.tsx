...
export const Clip = forwardRef<HTMLDivElement, Props>(({ id, start = 0, duration = 5 }, ref) => {
  return (
    <div
      ref={ref}
      data-index={id}
      style={{ zIndex: 1, width: `${duration}%`, left: `${start}%` }}
      className='grid grid-flow-col absolute bg-blue-400 border border-blue-500 rounded-lg h-full cursor-grab active:cursor-grabbing'
      onContextMenu={(e) => {
        e.preventDefault();
      }}
    >
