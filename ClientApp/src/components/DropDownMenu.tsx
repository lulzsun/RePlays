...
export const DropDownMenu: React.FC<Props> = ({text, items, width="full", zIndex=50}) => {
    ...
    <div className={`w-${width}`} style={{zIndex: zIndex}}>
      ...
      type="button">
        ...
        <select className={`absolute w-full left-0 top-0 opacity-0`} onChange={(e) => {items?.find(item => item.name === e.target.value)?.onClick!();}}>
        {items && items.map((item, i) => {
          // @ts-ignore
          return <option key={item.name} tabIndex={i} value={item.name}
