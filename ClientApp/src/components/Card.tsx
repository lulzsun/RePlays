  ...
  return (
    ...
    >
      <div className='absolute z-50 w-full flex justify-between'>
        <div className={'m-2 group-hover:opacity-100 ' + (checked ? 'opacity-100' : 'opacity-0')}>
          <input
            ...
            type='checkbox'
            className='h-4 w-4'
            checked={checked === undefined || checked === false ? false : true}
            onChange={(e) => {
              onChange(e);
            }}
          />
      ...
      <Link
        ...
        to={`/player/${game.replaceAll("%", "%25")}/${video}/${videoType}`}
        onClick={() => {
          console.log(folder);
        }}
      >
        <div className='relative text-white w-full rounded-t-lg object-cover overflow-hidden items-center'>
          ...
          >
            {duration > 3600
              ? new Date(duration * 1000).toISOString().substr(11, 8).replace(/^0+/, '')
          ...
          <img className='absolute z-20 w-full' alt='' src={`${folder}/${game.replaceAll("%", "%25")}/.thumbs/${thumb}`} />
