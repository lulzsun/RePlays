...
function App() {
  ...
  const [contextMenuPosition, setContextMenuPosition] =
    useState<ContextMenuPosition>({ x: -100, y: -100 });
  ...
  function handleWebViewMessages(event: Event) {
    ...
    if (data && data.title !== "Downloading") console.log(message, data);
    ...
    switch (message) {
      ...
      case "UserSettings":
        ...
        localStorage.setItem(
          "availableRateControls",
          data.captureSettings.rateControlCache
        );
        localStorage.setItem(
          "availableFileFormats",
          JSON.stringify(data.captureSettings.fileFormatsCache)
        );
        ...
        >
          <div
            className={
              ...
              window.matchMedia("(prefers-color-scheme: dark)").matches
                ? "System"
                : userSettings?.generalSettings.theme) === "System"
                ? "Dark"
                : userSettings?.generalSettings.theme
            }
          >
            <Modal
              modalData={modalData}
            ...
            <div className="bg-white dark:bg-gray-800 relative min-h-screen lg:flex">
              <div
                className="absolute inline-block text-left dropdown"
                style={{ zIndex: 9999 }}
              >
                <input
                  tabIndex={-1}
                  ref={contextMenuFocusEle}
                  className="w-0"
                />
              ...
              <div
                ref={sideBarEle}
                style={{ zIndex: 999 }}
                className="flex flex-col h-screen justify-between bg-gray-900 text-blue-100 w-auto py-2 px-2 absolute inset-y-0 left-0 transform -translate-x-full lg:relative lg:translate-x-0 transition duration-200 ease-in-out"
              >
                ...
                <div className="flex flex-row items-center justify-center w-full">
                  <div
                    title={t("menuTitle01")}
                    className="p-2 px-4 flex items-center rounded transition duration-200 hover:bg-gray-700 hover:text-white cursor-pointer"
                  >
                    <svg
                      className="w-6 h-6"
                      xmlns="http://www.w3.org/2000/svg"
                  ...
                  <div
                    ...
                    className="p-2 px-4 flex items-center rounded transition duration-200 hover:bg-gray-700 hover:text-white cursor-pointer"
                    onClick={(e) => {
                      postMessage("ShowRecentLinks");
                      setRecentLinksMenuOpen(true);
                    }}
                  >
                    <svg
                      className="w-6 h-6"
              ...
              <div className="flex flex-col h-screen w-full text-2xl font-semibold text-gray-900 dark:text-white">
                <div className="flex-initial">
                  <div className="bg-gray-900 text-gray-100 flex justify-between lg:hidden">
                    <div className="block p-4 text-white font-semibold">
                      RePlays
                    </div>
                    <button
                      className="p-4 focus:outline-none focus:bg-gray-700"
                      onClick={() => {
                        sideBarEle.current!.classList.toggle(
                          "-translate-x-full"
                        );
                      }}
                    >
                ...
                <div className="flex-auto overflow-hidden h-full p-7 text-gray-900 dark:text-white">
                  <Switch>
                    ...
                    <Route exact path="/player/:game/:video/:videoType">
                      <Player
                        videos={sessions != null ? sessions.concat(clips) : []}
                      />
                    </Route>
                  </Switch>
                </div>
