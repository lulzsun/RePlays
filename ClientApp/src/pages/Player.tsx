...
const ZOOMS = [
  100, 110, 125, 150, 175, 200, 250, 300, 400, 500, 1000, 2000, 3000, 4000,
  5000, 7500, 10000,
];
...
export const Player: React.FC<Props> = ({ videos }) => {
  ...
  useEffect(() => {
    ...
    function handleOnMouseDown(e: MouseEvent) {
      ...
      if (e.button === 0) {
        if (element === seekBarElement.current) {
          ...
          if (e.detail === 1) {
            if (element === seekWindowElement.current) mouseSeek(e);
          } else if (e.detail === 2) {
            mouseSeek(e);
          }
        }
      ...
      if (
        clipsRef.current?.indexOf(element.parentElement as HTMLDivElement) !==
        -1
      ) {
        let index = clipsRef.current?.indexOf(
        ...
        if (e.button === 0) {
          ...
          clipDragOffset =
            e.clientX -
            clipsRef.current[clipDragging]?.getBoundingClientRect().left;
        } else if (e.button === 2) {
          // context menu / Delete Clip
          if (index != -1) handleDeleteClip(e, index);
        }
      } else if (
        bookmarksRef.current?.indexOf(element as HTMLDivElement) !== -1
      ) {
        // Bookmark deleting
        ...
        if (e.button === 0) {
          ...
          clipResizeLimit =
            clipsRef.current[clipDragging]?.clientWidth +
            clipsRef.current[clipDragging]?.offsetLeft;
        } else if (e.button === 2) {
          // context menu / Delete Clip
          if (index != -1) handleDeleteClip(e, index);
        }
    ...
    function handleOnMouseMove(e: MouseEvent) {
      ...
      if (clipDragging !== -1 && clipResizeDir === "") {
        let clickLeft =
          e.clientX -
          clipDragOffset +
          timelineElement.current!.scrollLeft -
          seekWindowElement.current!.offsetLeft;
        if (clickLeft < 0) clickLeft = 0;
        else if (
          clickLeft >
          seekWindowElement.current!.clientWidth -
            clipsRef.current[clipDragging].getBoundingClientRect().width
        )
          clickLeft =
            seekWindowElement.current!.clientWidth -
            clipsRef.current[clipDragging].getBoundingClientRect().width;
        clipsRef.current[clipDragging].style.left = `${
          (clickLeft / seekWindowElement.current!.clientWidth) * 100
        }%`;
        ...
        if (clipResizeDir === "right") {
          let clickLeft =
            ...
            timelineElement.current!.scrollLeft -
            seekWindowElement.current!.offsetLeft;
          if (clickLeft > seekWindowElement.current!.offsetWidth)
            clickLeft = seekWindowElement.current!.offsetWidth;
          clipsRef.current[clipDragging].style.width = `${
            (clickLeft / seekWindowElement.current!.clientWidth) * 100
          }%`;
          ...
          clipsRef.current[clipDragging].style.width = `${
            ((clipsRef.current[clipDragging].offsetWidth +
              (clipsRef.current[clipDragging].offsetLeft - clickLeft)) /
              seekWindowElement.current!.clientWidth) *
            100
          }%`;
          clipsRef.current[clipDragging].style.left = `${
    ...
    if (bookmarks.length !== 0) {
      let videoMetadata = JSON.parse(
        localStorage.getItem("videoMetadataBookmarks")!
      );
      videoMetadata[`/${video}`] = { bookmarks };
      localStorage.setItem(
        "videoMetadataBookmarks",
  ...
  useEffect(() => {
    seekBarElement.current!.style.left = `calc(${
      (currentTime / videoElement.current!.duration) * 100
    }% - 3px)`;
    targetSeekElement.current!.style.left =
      seekBarElement.current!.offsetLeft + 6 + "px";
    ...
    if (seekWindowElement.current && seekBarElement.current) {
      ...
      if (videoElement.current)
        newClips.push({
          id: Date.now(),
          start: start,
          duration: (10 / videoElement.current.duration) * 100,
        }); // 10 seconds
      setClips(newClips);
    }
  }
    ...
    contextMenuCtx?.setItems([
      {
        ...
        onClick: () => {
          let newClips = clips.slice();
          if (videoElement.current) newClips.splice(index, 1);
          setClips(newClips);

          let videoMetadata = JSON.parse(
            localStorage.getItem("videoMetadata")!
          );
    ...
    if (videoElement.current) {
      if (playbackClips === -1) {
        ...
        videoElement.current.currentTime =
          (clips.sort((a, b) => (a.start > b.start ? 1 : -1))[0].start / 100) *
  ...
  function handleVideoLoad(e: SyntheticEvent) {
    let videoMetadata = JSON.parse(localStorage.getItem("videoMetadata")!);
    let videoMetadataBookmarks = JSON.parse(
      localStorage.getItem("videoMetadataBookmarks")!
    );

    if (videoElement.current && volumeSliderElement.current) {
      videoElement.current.volume =
        parseInt(volumeSliderElement.current.value) / 100;
      videoElement.current.play();
    }
    ...
    if (playbackClips !== -1 && clips) {
      ...
      for (let index = 0; index < clips.length; index++) {
        ...
        if (
          ...
          start + duration > videoElement.currentTime
        ) {
      ...
      if (clips[_playbackClips] !== undefined) {
        ...
        var duration =
          (clips[_playbackClips].duration / 100) * videoElement.duration;
        if (videoElement.currentTime >= start + duration) {
          if (clips.length === _playbackClips + 1) {
            ...
            var nextStart =
              (clips[_playbackClips + 1].start / 100) * videoElement.duration;
  ...
  function mouseSeek(e: MouseEvent) {
    let clickLeft =
      ...
      seekWindowElement.current!.offsetLeft;
    if (clickLeft < 0) clickLeft = 0;
    else if (clickLeft > seekWindowElement.current!.clientWidth)
      clickLeft = seekWindowElement.current!.clientWidth;
    let newCurrentTime =
      (clickLeft / seekWindowElement.current!.clientWidth) *
      videoElement.current!.duration;
        ...
        folder?.includes("videos.replays.app") && (
          <div className="flex flex-col gap-2 font-medium text-base pb-2">
            <span className="font-normal text-sm">
              DEV ONLY NOTE: Player has issues that only occur when debugging,
      ...
      <div className="flex flex-initial h-20 grid grid-flow-row">
        ...
        >
          <div
            style={{
              height: "1.1rem",
              width: `calc(${ZOOMS[currentZoom]}% - 12px)`,
            }}
            className="inline-block mx-1.5 grid grid-flow-col bg-gray-400 border-gray-300 border-l-2"
          >
          ...
          <div
            ref={seekWindowElement}
            style={{
              height: "45px",
              width: `calc(${ZOOMS[currentZoom]}% - 12px)`,
            }}
            className="inline-block mx-1.5 relative bg-gray-300"
          >
            <div
              ref={seekBarElement}
              style={{ width: "6px", left: "-3px" }}
              className="z-30 absolute bg-red-500 rounded-lg h-full cursor-ew-resize"
            />
            {clips &&
              clips.map((clip, i) => {
                return (
          ...
          <div
            ...
            style={{ height: "calc(100%)", width: "6px", left: "3px" }}
            className="relative bg-green-500 rounded-lg h-full cursor-ew-resize"
          />
        </div>
      </div>
      ...
      <div className="flex flex-initial grid grid-flow-col">
        <div className="flex justify-start">
          <div className="border-2 rounded-b-lg">
            ...
            <button
              title={t("playerItem03")}
              className="justify-center w-auto h-full px-4 py-2 text-sm font-medium leading-5 text-gray-700 transition duration-150 ease-in-out bg-white hover:bg-gray-200 hover:text-gray-700 focus:outline-none focus:border-blue-300 focus:shadow-outline-blue active:bg-gray-50 active:text-gray-800"
              type="button"
              onClick={() => (videoElement.current!.currentTime -= 5)}
            >
              <svg
                xmlns="http://www.w3.org/2000/svg"
                width="16"
            ...
            <span className="-mt-0.5 mb-0.5 inline-block align-middle w-auto h-full px-4 py-2 text-sm font-medium leading-5 text-gray-700 transition duration-150 ease-in-out bg-white hover:bg-gray-200 hover:text-gray-700 active:bg-gray-50 active:text-gray-800">
              {`${secondsToHHMMSS(currentTime)} / ${secondsToHHMMSS(
        ...
        <div className="flex justify-center">
          {clips.length > 0 && (
            <div className="border-2 rounded-b-lg">
              <span
                title={`${
                  playbackClips === -1 ? t("playerItem01") : t("playerItem06")
                } ${t("playerItem07")}${clips.length > 1 ? "s" : ""}`}
                className="cursor-pointer -mt-0.5 mb-0.5 inline-block align-middle w-auto h-full px-4 py-2 text-sm font-medium leading-5 text-gray-700 transition duration-150 ease-in-out bg-white hover:bg-gray-200 hover:text-gray-700 active:bg-gray-50 active:text-gray-800"
              ...
              >
                {playbackClips === -1 ? (
                  <svg
                    ...
                    height="16"
                    fill="currentColor"
                    className="-mt-0.5 align-center mr-2 inline"
                    viewBox="0 0 16 16"
                  >
                ...
                {secondsToHHMMSS(
                  clips
                    .map(
                      (clip) =>
                        (clip.duration / 100) * videoElement.current!.duration
                    )
                    .reduce((prev, next) => prev + next)
                )}
              </span>
              <button
                title={`${t("playerItem08")}${clips.length > 1 ? "s" : ""}`}
                type="button"
                className="justify-center w-auto h-full px-4 py-2 text-sm font-medium leading-5 text-gray-700 transition duration-150 ease-in-out bg-white hover:bg-gray-200 hover:text-gray-700 focus:outline-none focus:border-blue-300 focus:shadow-outline-blue active:bg-gray-50 active:text-gray-800"
        ...
        <div className="flex justify-end">
          <div className="border-2 rounded-b-lg">
            ...
            >
              ...
              >
                <defs></defs>
                <title>{t("playerItem09")}</title>
                <g id="bookmark-Filled">
                  <path
                    id="bookmark-Filled-2"
                    data-name="bookmark-Filled"
                    d="M19.5,7V20a1.5,1.5,0,0,1-2.354,1.233l-4.863-3.367a.5.5,0,0,0-.57,0L6.854,21.231A1.5,1.5,0,0,1,4.5,20V7A4.505,4.505,0,0,1,9,2.5h6A4.505,4.505,0,0,1,19.5,7Z"
                  />
                </g>
              </svg>
            </button>
            ...
            {videoType === "Clips" ? (
              <button
                title={t("playerItem10")}
                className="justify-center w-auto h-full px-4 py-2 text-sm font-medium leading-5 text-white transition duration-150 ease-in-out bg-blue-400 hover:bg-blue-300 hover:text-gray-700 focus:outline-none focus:border-blue-300 focus:shadow-outline-blue active:bg-gray-50 active:text-gray-800"
                type="button"
            ...
            <span
              title={t("playerItem11")}
              className="text-center cursor-pointer -mt-0.5 mb-0.5 inline-block align-middle w-12 h-full px-4 py-2 text-sm font-medium leading-5 text-gray-700 transition duration-150 ease-in-out bg-white hover:bg-gray-200 hover:text-gray-700 active:bg-gray-50 active:text-gray-800"
              onClick={() => {
                if (currentZoom - 1 > -1) setZoom(currentZoom - 1);
              }}
            >
