...
export const Player: React.FC<Props> = ({ videos }) => {
  ...
  useEffect(() => {
    ...
    function handleOnMouseDown(e: MouseEvent) {
      ...
      } else if (bookmarksRef.current?.indexOf(element as HTMLDivElement) !== -1) {
        // Bookmark deleting
    ...
    function handleOnMouseMove(e: MouseEvent) {
      ...
      if (clipDragging !== -1 && clipResizeDir === '') {
        ...
        if (clipResizeDir === 'right') {
          ...
          let clickLeft =
            e.clientX + timelineElement.current!.scrollLeft - seekWindowElement.current!.offsetLeft;
          if (clickLeft < 0) clickLeft = 0;
    ...
    function handleOnMouseUp(e: MouseEvent) {
      ...
      if (clipDragging !== -1) {
        ...
        clipsCopy[clipDragging].start =
          (clipsRef.current[clipDragging].offsetLeft / seekWindowElement.current!.clientWidth) *
          100;
        clipsCopy[clipDragging].duration =
          (clipsRef.current[clipDragging].offsetWidth / seekWindowElement.current!.clientWidth) *
          100;
        setClips(clipsCopy);
        clipDragging = -1;
      }
  ...
  function mouseSeek(e: MouseEvent) {
    ...
    let newCurrentTime =
      (clickLeft / seekWindowElement.current!.clientWidth) * videoElement.current!.duration;
      ...
      <div className='flex flex-initial grid grid-flow-col'>
        <div className='flex justify-start'>
          <div className='border-2 rounded-b-lg'>
            <button
              title={`${videoElement.current?.paused ? t('playerItem01') : t('playerItem02')}`}
        ...
        <div className='flex justify-center'>
          {clips.length > 0 && (
            <div className='border-2 rounded-b-lg'>
              <span
                title={`${playbackClips === -1 ? t('playerItem01') : t('playerItem06')} ${t(
                  'playerItem07',
                )}${clips.length > 1 ? 's' : ''}`}
              ...
              >
                ...
                {secondsToHHMMSS(
                  clips
                    .map((clip) => (clip.duration / 100) * videoElement.current!.duration)
