...
export const VideosPage: React.FC<Props> = ({
  ...
  const [checkedVideos, setCheckedVideos] = useState(
    Array(videos != null ? videos.length : 0).fill(false),
  );
  ...
  useEffect(() => {
    if (
      videoType === 'Sessions' &&
      videos != null &&
      localStorage.getItem('videoMetadata') !== null
    ) {
      // this purges videoMetadata of sessions that do not exist anymore
  ...
  useEffect(() => {
    const timer = setTimeout(() => {
      if (customScrollParent) customScrollParent.scrollTop = scrollPos;
  ...
  function onVideoDelete() {
    ...
    checkedVideos.forEach((video, i) => {
      if (video === true) filePaths.push(`${videos[i].game}/${videos[i].fileName}`);
    });
    setCheckedVideos(Array(videos.length).fill(false));
    setCheckedLength(0);
    ...
    <div className='flex flex-col h-full border-0 border-b'>
      <div className='pb-4 flex-initial border-0 border-b'>
        ...
        {checkedLength > 0 && (
          <VideoDeleteControls
            ...
            selectAll={() => {
              setCheckedVideos(Array(videos.length).fill(true));
              setCheckedLength(videos.length);
            }}
            unSelectAll={() => {
              setCheckedVideos(Array(videos.length).fill(false));
              setCheckedLength(0);
      ...
      {videos != null ? (
        <div
          ...
          onScroll={(e) => setScrollPos((e.target as HTMLElement).scrollTop)}
          className='-ml-1 pl-1 h-full overflow-y-auto'
        >
          <VirtuosoGrid
            //@ts-ignore
            ...
            itemContent={(index) => (
              <Card
                ...
                kills={
                  videos[index].metadata.kills !== undefined
                    ? videos[index].metadata.kills
                    : undefined
                }
                assists={
                  videos[index].metadata.assists !== undefined
                    ? videos[index].metadata.assists
                    : undefined
                }
                deaths={
                  videos[index].metadata.deaths !== undefined
                    ? videos[index].metadata.deaths
                    : undefined
                }
                champion={
                  videos[index].metadata.champion !== undefined
                    ? videos[index].metadata.champion
                    : undefined
                }
                win={
                  videos[index].metadata.win !== undefined ? videos[index].metadata.win : undefined
                }
