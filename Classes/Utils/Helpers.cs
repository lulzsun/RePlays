...
namespace RePlays.Utils {
    public static class Functions {
        ...
        public static async Task<bool> DownloadLibObsAsync() {
            ...
            if (File.Exists(libObsDir))
                if (SHA256Compare(libObsDir, correctHash)) return true;

            Logger.WriteLine("libobs-windows64-release-27.5.32.7z missing or failed checksum, starting download");
            WebMessage.DisplayModal("Downloading libobs-windows64-release-27.5.32.7z from web.archive.org", "Downloading", "none", 0, 103194624);
            using (var client = new WebClient()) {
                client.DownloadProgressChanged += (o, args) => {
                    WebMessage.DisplayModal("Downloading libobs-windows64-release-27.5.32.7z from obsstudios3.streamlabs.com", "Downloading", "none", args.BytesReceived, 103194624);
                };
                client.DownloadFileCompleted += (o, args) => {
                    Logger.WriteLine("Finished downloading libobs-windows64-release-27.5.32.7z");
                    WebMessage.DisplayModal("Finished downloading libobs-windows64-release-27.5.32.7z, doing a checksum", "Downloading", "info");
                };
                await client.DownloadFileTaskAsync(
                    new Uri("https://obsstudios3.streamlabs.com/libobs-windows64-release-27.5.32.7z"),
                    libObsDir);
            }
        ...
        public static Task<bool> InstallLibObs() {
            ...
            startInfo.Arguments = "x \"" + libObsDir + "\" -aoa -o\"" + GetTempFolder() + "\"";
            ...
            // place files/folder in root of application
            DirectoryCopy(libObsDir + @"\bin\64bit", Application.StartupPath, true);
        ...
        public static Task<bool> InstallPlaysSetup() {
            ...
            if (File.Exists(playsSetup) && !File.Exists(Path.Join(GetTempFolder(), @"\Plays-3.0.0-full.nupkg"))) {
                startInfo.Arguments = "e \"" + playsSetup + "\" -aoa -o\"" + GetTempFolder() + "\" Plays-3.0.0-full.nupkg";
                install = true;
            }
